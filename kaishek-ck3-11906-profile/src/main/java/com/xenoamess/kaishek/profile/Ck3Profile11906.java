...
public final class Ck3Profile11906 implements KaishekProfile {
    ...
    private static final List<OpcodeDescriptor> DESCRIPTORS = List.of(
            ...
            descriptor("has_character_modifier", OpcodeKind.TRIGGER, InputType.STRING, ScopeType.CHARACTER,
                    List.of(), RandomnessClass.DETERMINISTIC, false, true),
            // Exact-build 1.19.0.6 evidence identifies this as a scalar
            // Character boolean trigger backed by the active CAccolade
            // identity. Keep the descriptor syntax/profile-only; recognizing
            // the shape does not certify native store resolution or runtime
            ...
            descriptor("war_days", OpcodeKind.TRIGGER, InputType.INTEGER, ScopeType.WAR,
                    List.of(), RandomnessClass.DETERMINISTIC, false, true),
            // Exact-build 1.19.0.6 evidence identifies this as scalar
            // innovation-key membership on the current Culture scope.  Keep
            // the descriptor syntax/profile-only; recognizing the shape does
            // not certify native lookup, ownership, or runtime semantics.
            descriptor("has_innovation", OpcodeKind.TRIGGER, InputType.STRING, ScopeType.CULTURE,
                    List.of(), RandomnessClass.DETERMINISTIC, false, true),
            // Exact-build 1.19.0.6 evidence identifies this as scalar
            // cultural-tradition-key membership on the current Culture
            // scope. Keep the descriptor syntax/profile-only; recognizing
            // the shape does not certify native lookup, ownership, or runtime
            // semantics.
