...
java -jar target/kaishek-cli-0.1.0-SNAPSHOT.jar corpus --require-corpus path/to/mod
```
...
For acceptance runs that depend on an external mod tree, pass
`--require-corpus`. A missing directory or a directory with no `.txt`/`.gui`
files then emits stable JSON with `status: "SKIP"`, an explicit `reason`, and
exit code `1`; this prevents an unavailable external input from being counted
as a passing inventory. Without the flag, the existing optional `corpus`
behaviour is retained for compatibility.
